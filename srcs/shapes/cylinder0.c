...
/*   Updated: 2025/01/15 12:07:43 by trazanad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	get_cylinder_pt_distance(t_ray ray, t_vec3 center, t_vec3 axis_vec, float diameter, float height)
{
	...
	a = vec3_dot_product(ray.direction, ray.direction) - powf(vec3_dot_product(ray.direction, axis_vec), 2.0);
	c = vec3_dot_product(x,x) - powf(vec3_dot_product(x, axis_vec), 2.0) - powf(radius, 2.0);
	b = 2 * (vec3_dot_product(ray.direction, x) - (vec3_dot_product(ray.direction, axis_vec) * vec3_dot_product(x, axis_vec)));
	...
	m1 = vec3_dot_product(ray.direction, vec3_scalar_mult(axis_vec, x1)) + vec3_dot_product(x, axis_vec);
    m2 = vec3_dot_product(ray.direction, vec3_scalar_mult(axis_vec, x2)) + vec3_dot_product(x, axis_vec);
...
float	get_point_heigh(t_ray ray, t_vec3 axis_vec, t_vec3 cylinder_center, float distance)
{
	...
	m = vec3_dot_product(ray.direction, vec3_scalar_mult(axis_vec, distance)) + vec3_dot_product(x, axis_vec);
	return (m);
}
...
t_vec3 get_cylinder_normal(t_ray ray, float distance, t_vec3 cylinder_center, t_vec3 axis_vec)
{
	...
	tmp = vec3_add(cylinder_center, vec3_scalar_mult(axis_vec, m));
...
static int get_diffuse_light_color(t_ray ray, float distance, t_vec3 cylinder_center, t_vec3 axis_vec)
{
    ...
    t_vec3 light_pos = vec3_create(500, 100, 0);
